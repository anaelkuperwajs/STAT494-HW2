...
Link: https://github.com/anaelkuperwajs/STAT494-HW2
...
ggplot(lending_club, aes(x = verification_status)) +
  geom_bar()

ggplot(lending_club, aes(x = addr_state)) +
  geom_bar()

ggplot(lending_club, aes(x = Class)) +
  geom_bar()

ggplot(lending_club, aes(x = sub_grade)) +
  geom_bar()

ggplot(lending_club, aes(x = annual_inc)) +
  geom_histogram()

ggplot(lending_club, aes(x = delinq_2yrs)) +
  geom_bar()

ggplot(lending_club, aes(x = acc_now_delinq)) +
  geom_bar()

# lending_club %>%
#   count(acc_now_delinq, Class) %>%
#   View()

ggplot(lending_club, aes(x = delinq_amnt)) +
  geom_histogram()

# lending_club %>%
#   count(delinq_amnt, Class) %>%
#   View()
```
...
lending_club_split <- initial_split(lending_club_mod, prop = 3/4, strata = Class)
lending_club_split
...
lending_club_recipe <- recipe(Class ~ ., data = lending_club_training) %>%
  step_mutate_at(has_type(match = "integer"), fn = ~as.numeric(.)) %>%
  step_mutate(sub_grade = as.factor(str_sub(sub_grade, end = 1))) %>% 
  step_mutate(addr_state = fct_collapse(addr_state,
                           West = c("AK", "AZ", "CA", "CO", "HI", "ID", "MT", "NM", "NV", "OR", "UT", "WA",
                                    "WY"),
                           South = c("AL", "AR", "DC", "DE", "FL", "GA", "KY", "LA", "MD", "MS", "NC", "OK",
                                     "SC", "TN", "TX", "VA", "WV"),
                           Northeast = c("CT", "MA", "ME", "NH", "NJ", "NY", "PA", "RI", "VT"),
                           Midwest = c("IL", "IN", "KS", "MI", "MO", "MN", "ND", "NE", "OH", "SD", "WI"))) %>%
  step_normalize(all_numeric(), -all_outcomes()) %>% 
  step_dummy(all_nominal(), -all_outcomes())

prep(lending_club_recipe) %>% 
  juice()
```
...
lending_club_lasso_tune %>% 
  ...
  scale_x_log10(
   breaks = scales::trans_breaks("log10", function(x) 10^x),
   labels = scales::trans_format("log10",scales::math_format(10^.x))) +
  labs(x = "penalty", y = "ROC")

collect_metrics(lending_club_lasso_tune) %>% 
  filter(.metric == "accuracy") %>% 
  arrange(desc(mean))
...
```{r}
#define model
ranger_spec <- rand_forest(mtry = tune(), min_n = tune(), trees = 100) %>% 
  set_mode("classification") %>%
  set_engine("ranger")
...
rf_grid <- grid_regular(finalize(mtry(), lending_club_training %>% select(-Class)),
                        min_n(),
                        levels = 3)
...
They are both similar, with the best means being approximately 0.99 for both accuracy and area under the ROC curve. The best tuning parameter for the area under the ROC curve is slightly better, with a mean of 0.9972544
...
best_param_acc <- ranger_tune %>% 
  select_best(metric = "accuracy")

best_param_roc <- ranger_tune %>% 
...
Something that is interesting is that there is a huge spike around the residual of 0, with majority of values being more positive than negative. It doesn't look like there are any odd values and the boxplot shows how the absolute value of all the residuals is very small, under 0.1. The center is close to 0 and the spread is not very large.

```{r}
ranger_final_model <- ranger_workflow %>%
  finalize_workflow(best_param_roc) %>%
  fit(data = lending_club_training)

rf_explain <-
  explain_tidymodels(
    model = ranger_final_model,
...
The four most important variables are int_rate, addr_state, open_il_12m, annual_inc.
...
```{r}
set.seed(494)
rf_pdp <- model_profile(explainer = rf_explain, variables = c("annual_inc", "int_rate", "open_il_12m"))
...
set.seed(494)
rf_pdp2 <- model_profile(explainer = rf_explain, variables = "addr_state")
...
It appears to be that the random forest model is contributing the most.
...
17. Fit the final stacked model using `fit_members()`. Apply the model to the test data and report the accuracy and area under the curve. Create a graph of the ROC and construct a confusion matrix. Comment on what you see.

What I notice is that the model is extremely good, with a near perfect ROC curve and very few incorrect predictions shown in the confusion matrix. This can be alarming, since it is very close to perfect. The area under the ROC curve is 0.9998821 and the accuracy is 0.9984443, very close to one.
...
saveRDS(lending_club_final_stack, "final_model.rds")


lending_club_predictions <- lending_club_testing %>% 
  bind_cols(lending_club_final_stack %>% 
  predict(new_data = lending_club_testing)) %>% 
  bind_cols(lending_club_final_stack %>% 
              predict(new_data = lending_club_testing, type = "prob"))

lending_club_predictions %>% 
  ggplot(aes(x = .pred_good, 
             y = Class)) +
  geom_boxplot() +
  labs(x = "Good Predictions", 
       y = "Actual Class")

lending_club_predictions %>% 
  ggplot(aes(x = .pred_bad, 
             y = Class)) +
  geom_boxplot() +
  labs(x = "Bad Predictions", 
       y = "Actual Class")
...
autoplot(roc_curve(lending_club_predictions, truth = Class, estimate = .pred_bad))

conf_mat(lending_club_predictions, truth = Class, estimate = .pred_class)
```
...
Website page link: https://clever-shaw-7eff52.netlify.app/shiny.html

Shiny app link: https://anaelkuperwajs.shinyapps.io/STAT494-HW2-ShinyApp/

Github repository: https://github.com/anaelkuperwajs/STAT494-HW2-ShinyApp
...
This film was incredibly impactful and did a great job introducing the concepts, providing powerful examples, and showing the real people that bias in technology affects. This film made it easy to understand why code is biased and how people embed their own bias into technology as it is created. I was very familiar with much of this information before watching the film, such as the fact that technology is biased, in what ways it is biased, and how biased data is leading to a lot of the issues. One of the pieces I was less familiar with and that surprised me was the policy aspect. The part where they show the surveillance in England that is scanning civilians' faces on the street and comparing against a database of possible offenders showed the ways in which this technology is used in unfair ways in our everyday lives. Furthermore, when the activist protested and spoke up, the police defended their way of using this technology. The end of the movie, also, shows a trial with Representative Alexandria Ocasio Cortez and it is surprising to see this being discussed in such modern times. While it is logical that such a trial would have been recent, also given the release date of this film is this month, sometimes it is hard to believe it is still a discussion. People still badly want to believe technology is neutral, and I am glad to see films like these providing evidence and examples that it is not, and there is a lot of work to be done. I experienced great frustration throughout this film, but I also felt hopeful that people might begin to understand the great caution we must use when we go about dealing with technology.
